...
void Snake::move(int magpieX, int magpieY) { //newX와 newY는 사용하지 않음
    ...
    // 맵 범위 검증
    if (newHeadX < 0 || newHeadX >= screenWidth / GRID_stage3 //맵 너비 검증
        || newHeadY < 1 || newHeadY >= screenHeight / GRID_stage3 || isNodeInList(newHeadX, newHeadY))//맵 높이 검증)
    {
...
void Bell::spawn() {
    ...
    uniform_int_distribution<int> distributionY(0, screenHeight / GRID_stage3 - 1);
